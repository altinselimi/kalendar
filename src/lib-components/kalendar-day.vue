  ...
  methods: {
    ...
    getDayEvents(events) {
      ...
      return myWorker
        ...
        .then(constructed_events => this.day_events = constructed_events);
    },
    clearCreatingLeftovers() {
      for (let [key, val] in Object.entries(this.day_events)) {
        let hasPending = val.some(
                e => e.status === "popup-initiated" || e.status === "creating"
        );
        if (hasPending) {
          let completed = val.filter(event => event.status === "completed");
    ...
    validateSelection(event) {
      let { original_starting_cell, current_cell } = event;
      if (
    ...
    makeEvent(status) {
      let {ending_cell, starting_cell,} = this.creator;
    ...
    initiatePopup() {
      ...
      let { starting_cell } = this.creator;
    ...
    overlapPolice(payload) {
      ...
      let overlapped = Object.keys(this.day_events)
        .map(evKey => this.day_events[evKey])
        .flat()
        .filter(event => {
      ...
      let [needed, offset, updates] = payload.direction === "reverse" ?
              ['end', 0, ['starting_cell', 'current_cell']] :
              ['start', -1, ['ending_cell',]];

      let needed_cell = overlapped[0][needed];
      let event_cell = this.day_cells.find(
        c => c.value === needed_cell.masked_value
      );
      let cell = this.day_cells[event_cell.index + offset];
